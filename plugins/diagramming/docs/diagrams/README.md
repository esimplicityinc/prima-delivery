...
These four diagrams are referenced in PR [#5](https://github.com/esimplicityinc/prima-delivery/pull/5) as proof-of-product. All were generated by this plugin (drawio renderer + lane auto-stagger + WCAG AA contrast validation) — same code path, no manual edits.

> The RIC-401 appendix diagrams (`authority-map`, `hypothesis-lifecycle`, `update-protocols`, `pattern-extraction`, `artifact-schema`) were originally shipped here but were moved out of this PR's scope (RIC-353). They will be re-introduced in a follow-up PR scoped to RIC-401.
...
## shine-portal-deployment-v2.{png,svg}

The SHINE portal deployment topology, rendered by `/architecture-diagrams` against `~/Desktop/content-portal` after the v2.0.0 SKILL.md rewrite (commit `fbe2d90`). Verification artifact for the architectural shift that resolves @tommymason7's PR #5 review feedback. Six color-coded lanes (SOURCE → GITHUB ACTIONS → BUILD JOB → DEPLOY JOB → DATABRICKS APPS RUNTIME → POST-DEPLOY + USERS), sublabels per node, semantic 5-category arrow legend.

Spec: composed by the LLM following `fireworks-tech-graph`'s SKILL.md vocabulary (no static spec.json — free-form SVG composition path).
Renderer: `fireworks-tech-graph` (default) via this plugin's coordinator skill.
