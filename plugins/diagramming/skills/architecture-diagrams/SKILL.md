...
# Export PNG via the drawio CLI (brew install --cask drawio):
...
If `drawio` CLI is not on PATH, deliver the `.drawio` file alone and tell the user to open it in diagrams.net web (drag-drop) or any drawio editor.
